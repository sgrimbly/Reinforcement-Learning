...
# At the start of each search, we add dirichlet noise (https://en.wikipedia.org/wiki/Dirichlet_distribution) to the prior of the root to encourage the search to explore new actions.
def add_exploration_noise(config: MuZeroConfig, node: Node):
    actions = list(node.children.keys)
    noise = numpy.random.dirichlet([config.root_dirichlet_alpha] * len(actions))
    frac = config.root_exploration_fraction
    for a, n in zip(actions, noise): # Python’s zip() function creates an iterator that will aggregate elements from two or more iterables. 
...
def run_mcts(config: MuZeroConfig, root: Node, action_history: ActionHistory, network: Network):
    ...
    for _ in range(config.num_simulations):
        ...
        backpropagate(search_path, network_output.value, history.to_play(), config.discount, min_max_steps)
